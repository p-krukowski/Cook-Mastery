/*
  ...
  purpose: define enums, core tables, indexes, rls policies, and progress view
  affected: public.profiles, public.tutorials, public.articles, public.user_tutorials,
            public.user_articles, public.cookbook_entries, public.user_level_progress
  notes: rls is enabled on all new tables; tutorials/articles are readable only to authenticated users
*/

-- enum types for difficulty and tutorial categorization
do $$
begin
  create type public.difficulty_level as enum ('BEGINNER', 'INTERMEDIATE', 'EXPERIENCED');
exception
...
do $$
begin
  create type public.tutorial_category as enum ('PRACTICAL', 'THEORETICAL', 'EQUIPMENT');
exception
  when duplicate_object then null;
end $$;
...
create table if not exists public.profiles (
  ...
  selected_level public.difficulty_level not null default 'BEGINNER',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- tutorials table: curated learning content
create table if not exists public.tutorials (
  ...
  -- steps are structured as: [{"title": "...", "content": "...", "order": 1}]
...
-- articles table: theory and knowledge content
...
-- cookbook_entries: user-curated external recipes
...
-- enable rls for all tables created in this migration
...
-- anonymous users are denied profile reads
...
-- anonymous users are denied profile inserts
...
-- anonymous users are denied profile updates
...
-- anonymous users are denied tutorial reads
...
-- anonymous users are denied article reads
...
-- user_tutorials rls policies
-- authenticated users can read their own completion rows
...
-- anonymous users are denied reads
...
-- authenticated users can insert only their own completion rows
...
-- anonymous users are denied inserts
...
-- authenticated users can delete only their own completion rows
...
-- anonymous users are denied deletes
...
-- authenticated users can insert only their own entries
...
-- authenticated users can update only their own entries
...
-- anonymous users are denied updates
...
-- authenticated users can delete only their own entries
...
-- view for per-user, per-level completion progress
...
totals as (
  ...
  from (
    select level from public.tutorials
    union all
    select level from public.articles
  ) all_content
  group by level
),
user_levels as (
  select p.id as user_id, l.level
...
select
  ...
  case
    ...
    else round((coalesce(c.completed_count, 0)::numeric / t.total_count::numeric) * 100, 2)
  end as completion_percent,
  case
    when coalesce(t.total_count, 0) = 0 then false
    else (coalesce(c.completed_count, 0)::numeric / t.total_count::numeric) >= 0.85
  end as is_up_to_date
from user_levels ul
left join totals t on t.level = ul.level
left join completed c on c.user_id = ul.user_id and c.level = ul.level;
