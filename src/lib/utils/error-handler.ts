 ...
 */
export function createErrorResponse(code: string, message: string, details?: Record<string, string>): ApiErrorResponse {
  return {
    error: {
      code,
      message,
...
export function formatZodError(error: ZodError): ApiErrorResponse {
  ...
  return createErrorResponse("VALIDATION_ERROR", "Validation failed", details);
}
